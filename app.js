...
function renderQuiz(picked){
  ...
  btn.onclick = () => { const answers = [];
    picked.forEach((q, idx) => {
      const sel = [...document.querySelectorAll(`input[name="q${idx}"]`)].find(i=>i.checked); answers.push(sel?Number(sel.value):-1);
    ...
    // احسب الدرجة المئوية
    let correctCount = 0;
    picked.forEach((q, i) => { if (answers[i] === Number(q.correct)) correctCount++; });
    const score = Math.round((correctCount / picked.length) * 100);

    // تحديد القسم: إن كانت كل الأسئلة من قسم واحد استخدمه، وإلا mixed
    const detectSection = () => {
      const tags = picked.map(q => (q.tags && q.tags[0]) || "");
      // نحاول قراءة "section" إن تم حفظه في عنصر السؤال، وإلا نتركه فارغ
      // بما أن quiz_bank ليس فيه section صريح، سنمرره من دوال البناء.
      return window.__QUIZ_SECTION__ || "mixed";
    };

    recordResult({ section: detectSection(), score, total: picked.length });
    renderSummary();

  };
  cont.appendChild(btn);
}
function buildQuiz(section){ window.__QUIZ_SECTION__ = section;
  const pool = state.bank[section] || []; renderQuiz(pickRandom(pool, 5));
}
function buildFullQuiz(){ window.__QUIZ_SECTION__ = 'mixed';
...
const STORE_KEY = "tajweedProgress";
...
function recordResult(payload){
  const hist = loadProgress();
  hist.push({ ts: Date.now(), ...payload });
  saveProgress(hist);
}
function summarizeProgress(){
  const hist = loadProgress();
  const total = hist.length;
  const bySect = { noon_tanween:[], meem_sakinah:[], madd:[], mixed:[] };
  hist.forEach(h => {
    const key = h.section || "mixed";
    (bySect[key]||bySect.mixed).push(h);
  });
  function avg(arr){ if(!arr.length) return 0; return Math.round(arr.reduce((s,x)=>s+x.score,0)/arr.length); }
  function last(arr){ return arr.length ? arr[arr.length-1].score : 0; }
  const summary = {
    total,
    avgAll: avg(hist),
    lastAll: last(hist),
    sections: {
      noon_tanween: { count: bySect.noon_tanween.length, avg: avg(bySect.noon_tanween), last: last(bySect.noon_tanween) },
      meem_sakinah: { count: bySect.meem_sakinah.length, avg: avg(bySect.meem_sakinah), last: last(bySect.meem_sakinah) },
      madd: { count: bySect.madd.length, avg: avg(bySect.madd), last: last(bySect.madd) },
      mixed: { count: bySect.mixed.length, avg: avg(bySect.mixed), last: last(bySect.mixed) },
    },
    spark: hist.slice(-20).map(h=>h.score) // last 20 scores
  };
  return summary;
}
function renderSparkline(scores){
  if (!scores.length) return `<svg class="spark"></svg>`;
  ...
  const step = (W - 2*pad) / Math.max(1, scores.length-1);
  let d = "";
  scores.forEach((v,i)=>{
    const x = pad + i*step;
    const y = norm(v);
  ...
  return `<svg class="spark" viewBox="0 0 ${W} ${H}" preserveAspectRatio="none">
    <polyline fill="none" stroke="black" stroke-width="2" points="${
      scores.map((v,i)=>`${pad + i*step},${norm(v)}`).join(' ')
    }" />
  </svg>`;
}
function renderSummary(){
  ...
  const cards = (label, data) => `
    <div class="sum-card">
      <div class="small">${label}</div>
      <div class="kpi">${data.avg}% متوسط</div>
      <div class="small">آخر نتيجة: ${data.last}% — محاولات: ${data.count}</div>
    </div>`;
  el.innerHTML = `
    <div class="summary-grid">
      <div class="sum-card">
        <div class="small">المجموع</div>
        <div class="kpi">${s.avgAll}% متوسط عام</div>
        <div class="small">آخر نتيجة: ${s.lastAll}% — عدد الاختبارات: ${s.total}</div>
        ${renderSparkline(s.spark)}
      </div>
      ${cards("النون الساكنة والتنوين", s.sections.noon_tanween)}
      ${cards("الميم الساكنة", s.sections.meem_sakinah)}
      ${cards("أحكام المدود", s.sections.madd)}
    </div>
  `;
}
...
async function init(){
  ...
  $("showSummaryBtn").onclick = renderSummary;
  $("resetSummaryBtn").onclick = resetSummary;
  renderSummary();
