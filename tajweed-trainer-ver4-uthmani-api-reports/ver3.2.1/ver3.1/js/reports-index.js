...
(function(){
  ...
  function render(){
    ...
    if(q){
      idx = idx.filter(x=> (`${x.reportId||''} ${x.trainee||''} ${x.dateISO||''}`).toLowerCase().includes(q) );
    }
    idx.sort((a,b)=>{
    ...
    $('#empty').hidden = idx.length>0;

    idx.forEach(row=>{
      const tr = document.createElement('tr');
      const rid = row.reportId;
      const url = buildURL(rid);
      tr.innerHTML = `<td><code>${rid}</code></td>
        <td>${row.trainee||'—'}</td>
        <td>${fmtDate(row.dateISO)}</td>
        <td>${row.total||0}</td>
        <td>
          <a class="btn" href="${url}">فتح</a>
          <button class="btn" data-copy="${url}">نسخ الرابط</button>
          <button class="btn btn-danger" data-del="${rid}">حذف</button>
        </td>`;
      tbody.appendChild(tr);
    });

    $$('button[data-copy]').forEach(b=> b.onclick = async ()=>{
      const link = b.getAttribute('data-copy');
      try{ await navigator.clipboard.writeText(link); alert('تم نسخ الرابط'); }
      catch{ prompt('انسخ الرابط:', link); }
    });
    $$('button[data-del]').forEach(b=> b.onclick = ()=>{
      const rid = b.getAttribute('data-del');
      if(!confirm(`حذف التقرير ${rid}؟`)) return;
      try{ localStorage.removeItem(`tajweedy:report:${rid}`); }catch{}
      const idx = getIndex().filter(x=>x.reportId!==rid);
      setIndex(idx); render();
    });
  }
  ...
  function bind(){
    ...
    $('#exportCSV').onclick = ()=>{
      ...
      const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'tajweedy-reports.csv'; a.click();
      setTimeout(()=>URL.revokeObjectURL(a.href), 1500);
    };
    $('#clearAll').onclick = ()=>{
      if(!confirm('سيتم حذف جميع التقارير المحلية. متأكد؟')) return;
      const idx = getIndex();
      idx.forEach(x=>{ try{ localStorage.removeItem(`tajweedy:report:${x.reportId}`); }catch{} });
      setIndex([]); render();
    };
  }
